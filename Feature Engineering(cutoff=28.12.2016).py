...
#log_trimmed = pd.read_pickle('log_trimmed.pickle')
#members_trimmed = pd.read_pickle("members_trimmed.pickle")
#trans_trimmed = pd.read_pickle("trans_trimmed.pickle")
#trans = pd.read_pickle("trans.pickle")
#members_trimmed.to_pickle('members_trimmed.pickle')
#X = pd.read_pickle('X.pickle')

cutoff_date = pd.Timestamp('20161025') 
...
#Create list of members whose latest exp_date is between cutoff_date & cut_off_start_date (last 30 days)
...
# Legacy: irrelevant (delete this line): temp['last_date_churn'] =  temp.membership_expire_date.apply(lambda x: 1 if x<pd.Timestamp('20170101') else 0) 
...
X = X.fillna(0)
#Legacy: X['times_churn'] = X['times_churn'] + X.last_date_churn


#4. Calculating sum of actual active days
trans_trimmed.sort_values(['msno','transaction_date'], inplace=True)

trans_2d = trans_trimmed[['msno','transaction_date','membership_expire_date']] #for performance
temp = pd.DataFrame()
temp['msno'] = 0 
temp['days'] = 0
temp={}
...
while i<len(trans_2d):
    ...
    if trans_2d.iloc[i].msno == trans_2d.iloc[i+1].msno: #next line is still the same member
        ...
        days = 0
        i+=1
#temp= pd.read_pickle("activedays.pickle")
temp = pd.DataFrame.from_dict(temp, orient='index')
#temp.to_pickle('activedays.pickle')
...
##########################################upload Members table ####################################
members = pd.read_pickle('members.pickle')

# Convert date & time format
members_trimmed = members[members.msno.isin(X.msno)]
#members_trimmed.to_pickle('members_trimmed.pickle')
